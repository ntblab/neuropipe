...
For skyra, type s for the $scanner. For allegra, type a (e.g. scripts/retrieve-data-from-dicom.sh s)
...
If you cannot find your scans, you can look manually: 
	...
	All the ALLEGRA files are found in:
		/jukebox/dicom/conquest/Allegra-MRC20413/Turk-B/$YEAR/
...
if [ "$scanner" = s ]; then
	...
	SCANNER_DIR='Allegra-MRC20413'

else
	echo
	echo 'ERROR: you must specify which scanner to retrieve files from. For skyra, write an s, and for allegra, write an a. For example: 
